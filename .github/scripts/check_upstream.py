...
VCPKG_EXE = os.environ.get("VCPKG_EXE", "vcpkg")
...
def get_latest_tag_github(repo):
    # Try latest release first as it is more reliable for "latest version"
    ...
    try:
        with urllib.request.urlopen(req) as r:
            ...
            # We still need the commit SHA of this tag.
            # Get the tag reference from git refs
            tag_ref_url = f"{GITHUB_API_URL}/repos/{repo}/git/ref/tags/{tag_name}"
            try:
                tag_req = make_github_request(tag_ref_url)
                with urllib.request.urlopen(tag_req) as tag_r:
                    tag_data = json.loads(tag_r.read().decode())
                    sha = tag_data['object']['sha']
                    # If it's an annotated tag, we should resolve it to the commit sha
                    if tag_data['object']['type'] == 'tag':
                        tag_obj_url = tag_data['object']['url']
                        obj_req = make_github_request(tag_obj_url)
                        with urllib.request.urlopen(obj_req) as obj_r:
                            obj_data = json.loads(obj_r.read().decode())
                            sha = obj_data['object']['sha']
                    return tag_name.lstrip('v'), sha
            except Exception:
                # If resolving reference fails, fallback to tags API
                pass
    except Exception:
        pass

    # Fallback to tags endpoint if no official releases or if release lookup failed
...
def get_latest_tag_gitlab(gitlab_url, repo):
    ...
    if not gitlab_url:
        gitlab_url = GITLAB_API_URL
    url = f"{gitlab_url}/api/v4/projects/{encoded_repo}/repository/tags?order_by=updated"
    req = urllib.request.Request(url, headers={"User-Agent": "vcpkg-registry-bot"})
    try:
        with urllib.request.urlopen(req) as r:
            data = json.loads(r.read().decode())
...
def get_actual_sha512(port_name):
    # Run vcpkg install with --only-downloads to get the correct hash from the error message
    cmd = [VCPKG_EXE, "install", port_name, "--overlay-ports=ports", "--binarysource=clear", "--only-downloads"]
...
def update_port_files(port_name, port_dir, new_version, version_field, new_ref, is_github, block_content, parsed_args):
    ...
    print(f"[{port_name}] Wrote dummy SHA512. Triggering vcpkg download to obtain actual SHA512...")
...
def main():
    ...
    # Save original branch/commit to revert to after updates
    orig_branch_proc = subprocess.run(["git", "rev-parse", "--abbrev-ref", "HEAD"], capture_output=True, text=True)
    orig_branch = orig_branch_proc.stdout.strip()
    if orig_branch == "HEAD":
        orig_branch_proc = subprocess.run(["git", "rev-parse", "HEAD"], capture_output=True, text=True)
        orig_branch = orig_branch_proc.stdout.strip()
    ...
    for port in updated_ports:
        ...
        branch_name = f"update-{name}-{today_str}"
